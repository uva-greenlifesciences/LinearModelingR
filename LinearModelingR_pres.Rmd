...
slm <- lm(y ~ x)
abline(slm)
text(1, 22, bquote(Y == .(round(coef(slm)[1],2)) + .(round(coef(slm)[2],2))*X),
     cex = 1.4)
par(opar)
...
3. `pairs(mydata)`: All pairwise scatter plots 
...
1. CI of predicted mean (predicted _mean_ PSA given these predictors)
2. CI of predicted value (predicted PSA _value for an individual_ with given predictors)
...
Linear Model: Estimate effects of factor levels on response

ANOVA: assess whether or not factors affect response

Do ANOVA same way you fit linear model:
...
Can plot effects of factors with `plot.design`.  
Example: `plot.design(Y ~ factor, data = mydata)`
...
Compare nested models with a _partial F-test_ using the `anova` function.
...
We use logistic regression to model the _log odds_ of binary response.

Let $p$ be the probability that response is 1. Then the linear model is
